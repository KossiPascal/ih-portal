...
export class ChwsDrugComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.Months$ = Functions.months();
    this.Years$ = Functions.getYearsList();

    this.year$ = Functions.currentYear(this.initDate.end_date);
    this.month$ = Functions.currentMonth(this.initDate.end_date);
  ...
  genarateSites() {
    this.site$ = [];
    this.chw$ = [];
    const dist: string[] = Functions.returnEmptyArrayIfNul(this.chwsDrugDataForm.value.districts);
  ...
  genarateChws() {
    const sites: string[] = Functions.returnEmptyArrayIfNul(this.chwsDrugDataForm.value.sites);
    this.chw$ = [];
    this.chwsDrugDataForm.value.chws = [];
    if (notNull(sites))
  ...
  ParamsToFilter(): FilterParams {
    ...
    if (!this.roles.isChws()) {
      districts = Functions.returnEmptyArrayIfNul(this.chwsDrugDataForm.value.districts);
      sites = Functions.returnEmptyArrayIfNul(this.chwsDrugDataForm.value.sites);
      chws = Functions.returnEmptyArrayIfNul(this.chwsDrugDataForm.value.chws);
    } else {
    ...
    const prevM = Functions.previousMonth(_moth);
