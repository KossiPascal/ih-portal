...
//   constructor(private store: AppStorageService, private auth: AuthService, private sync: SyncService, private el: ElementRef, private renderer: Renderer2) {
...
//       this.PresentPersonsList = this.PresentPersonsList.filter(p => !((Object.keys(p).map(Number)).includes(teamId)));
//       this.PresentPersonsList.push({ [teamId]: this.Person$.filter(p => p.id && report.present_persons_ids && (report.present_persons_ids.map(String)).includes(`${p.id}`)) });
     
//       this.AbsentPersonsList = this.AbsentPersonsList.filter(p => !((Object.keys(p).map(Number)).includes(teamId)));
//       this.AbsentPersonsList.push({ [teamId]: this.Person$.filter(p => p.id && report.absent_persons_ids && (report.absent_persons_ids.map(String)).includes(`${p.id}`)) });
//     }
//   }
...
//     const presentPersonIds: number[] = this.PresentPersonsList
//       .flatMap(tpm => Object.values(tpm).flat())
...
//   getAbsentPersonsIds(){
//     const absentPersonIds: number[] = this.AbsentPersonsList
//       .flatMap(tpm => Object.values(tpm).flat())
//       .filter(person => typeof person.id === 'number')
//       .map(person => person.id as number);
...
//   AllFieldAreNotEmpty(teamId: number):boolean{
...
//     return notNull(title) || notNull(date) || notNull(start_hour) || notNull(end_hour)
//                       || notNull(presentPersonIds) || notNull(absentPersonIds) || notNull(otherPersons)
//                       || notNull(agenda) || notNull(recommandations) || notNull(discussion_topics) || notNull(next_steps);
...
//     }
//     if (!(DateUtils.compareTimes(start_hour, end_hour) <= 0)) {
//       i -= 1;
//       this.AddOrRemoveClassById(`start-hour-${teamId}`, 'custom-border');
//       this.AddOrRemoveClassById(`end-hour-${teamId}`, 'custom-border');
//     }
...
//       const isOk = DateUtils.compareTimes(start_hour, end_hour) <= 0;
...
//     const m = Functions.monthByArg(arg);
...
//       if (this.AbsentPersonIsSelected(psn, teamId)) {
//         this.AbsentPersonsList = this.AbsentPersonsList.map(teamPersons => {
//           const personsForTeam = teamPersons[teamId];
//           if (personsForTeam) return { ...teamPersons, [teamId]: personsForTeam.filter(p => p.id !== psn.id) };
//           return teamPersons;
//         });
//       }
//       //#####################
//       if (this.PresentPersonIsSelected(psn, teamId)) {
//         this.PresentPersonsList = this.PresentPersonsList.map(teamPersons => {
//           const personsForTeam = teamPersons[teamId];
//           if (personsForTeam) return { ...teamPersons, [teamId]: personsForTeam.filter(p => p.id !== psn.id) };
//           return teamPersons;
//         });
//       } else {
//         if (this.PresentPersonsList.length > 0) {
//           this.PresentPersonsList = this.PresentPersonsList.map(tp => {
//             if (tp[teamId]) return { ...tp, [teamId]: [...tp[teamId], psn] };
//             tp[teamId] = [psn];
//             return tp;
//           });
//         } else {
//           this.PresentPersonsList.push({ [teamId]: [psn] });
//         }
//       }
//       const tp = this.PresentPersonsList.find(tp => tp[teamId]);
//       if (tp) this.AddOrRemoveClassById(`present-persons-${teamId}`, 'custom-border', notNull(tp[teamId]));
//     }
//   }
...
//     if (this.CanUpdateReport(teamId)) {
//       if (this.PresentPersonIsSelected(psn, teamId)) {
//         this.PresentPersonsList = this.PresentPersonsList.map(pp => {
//           const ppft = pp[teamId];
//           if (ppft) return { ...pp, [teamId]: ppft.filter(p => p.id !== psn.id) };
//           return pp;
//         });
//       }
//       //#####################
//       if (this.AbsentPersonIsSelected(psn, teamId)) {
//         this.AbsentPersonsList = this.AbsentPersonsList.map(ap => {
//           const apft = ap[teamId];
//           if (apft) return { ...ap, [teamId]: apft.filter(p => p.id !== psn.id) };
//           return ap;
//         });
//       } else {
//         if (this.AbsentPersonsList.length > 0) {
//           this.AbsentPersonsList = this.AbsentPersonsList.map(tp => {
//             if (tp[teamId]) return { ...tp, [teamId]: [...tp[teamId], psn] };
//             tp[teamId] = [psn];
//             return tp;
//           });
//         } else {
//           this.AbsentPersonsList.push({ [teamId]: [psn] });
//         }
//       }
//       const tp = this.AbsentPersonsList.find(tp => tp[teamId]);
//       if (tp) this.AddOrRemoveClassById(`absent-persons-${teamId}`, 'custom-border', notNull(tp[teamId]));
//     }
//   }


//   PresentPersonIsSelected(pn: Person, teamId: number): boolean {
//     const tp = this.PresentPersonsList.find(tp => tp[teamId]);
//     return tp && tp[teamId] ? tp[teamId].some(p => p.id === pn.id) : false;
//   }

//   AbsentPersonIsSelected(pn: Person, teamId: number): boolean {
//     const tp = this.AbsentPersonsList.find(tp => tp[teamId]);
//     return tp && tp[teamId] ? tp[teamId].some(p => p.id === pn.id) : false;
//   }
...
//     // return p && (p.team as Team).id == teamId ? `${p.name}${i != l - 1 ? '&nbsp;&nbsp;|&nbsp;&nbsp;' : ''}` : '';
...
//   GetSelectedPresentPersonList(teamId: number): Person[] {
//     const tp = this.PresentPersonsList.find(tp => tp[teamId])
//     if (tp) return tp[teamId];
//     return [];
//   }

//   GetSelectedAbsentPersonList(teamId: number): Person[] {
//     const tp = this.AbsentPersonsList.find(tp => tp[teamId])
//     if (tp) return tp[teamId];
//     return [];
//   }
