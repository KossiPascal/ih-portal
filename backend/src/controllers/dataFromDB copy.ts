...
//   const chwsDrugData: { status: number, data: ChwsData[] } = await getChwsDataWithParams(req, res, next, true);
//   const chws: { status: number, data: Chws[] } = await getChws(req, res, next, true);

//   //if (chwsDrugData.status == 200 && chws.status == 200) {

//   var confirm: string[] = []
//   var dbChwsDrugData: { chwId: any, chw: Chws, data: ChwsDrugData }[] = [];

//   for (let i = 0; i < chws.data.length; i++) {
//     const asc = chws.data[i];
//     if (asc.id != null && asc.id != '' && !confirm.includes(asc.id!)) {
//       var chwsDrugOutPut: ChwsDrugData = {
//         Albendazole_400_mg_cp_1: getChwsDrugQantity(chwsDrugData.data, asc, 'alben_400', req, res),
//         Amoxiciline_250_mg_2: getChwsDrugQantity(chwsDrugData.data, asc, 'amox_250', req, res),
//         Amoxiciline_500_mg_3: getChwsDrugQantity(chwsDrugData.data, asc, 'amox_500', req, res),
//         Artemether_Lumefantrine_20_120mg_cp_4: getChwsDrugQantity(chwsDrugData.data, asc, 'lumartem', req, res),
//         Oral_Combination_Pills_5: getChwsDrugQantity(chwsDrugData.data, asc, 'pills', req, res),
//         Paracetamol_250_mg_6: getChwsDrugQantity(chwsDrugData.data, asc, 'para_250', req, res),
//         Paracetamol_500_mg_7: getChwsDrugQantity(chwsDrugData.data, asc, 'para_500', req, res),
//         Pregnancy_Test_8: getChwsDrugQantity(chwsDrugData.data, asc, 'pregnancy_test', req, res),
//         Sayana_Press_9: getChwsDrugQantity(chwsDrugData.data, asc, 'sayana', req, res),
//         SRO_10: getChwsDrugQantity(chwsDrugData.data, asc, 'sro', req, res),
//         TDR_11: getChwsDrugQantity(chwsDrugData.data, asc, 'tdr', req, res),
//         Vitamine_A_100000UI_12: getChwsDrugQantity(chwsDrugData.data, asc, 'vit_A1', req, res),
//         Vitamine_A_200000UI_13: getChwsDrugQantity(chwsDrugData.data, asc, 'vit_A2', req, res),
//         Zinc_14: getChwsDrugQantity(chwsDrugData.data, asc, 'zinc', req, res)
//       };


//       dbChwsDrugData.push({ chw: chws.data[i], data: chwsDrugOutPut });
//       confirm.push(asc.id!);
//     }
//   }

//   if (!dbChwsDrugData) return res.status(201).json({ status: 201, data: 'No data found !' });
//   return res.status(200).json({ status: 200, data: dbChwsDrugData });
//   // } else {
//   //   return res.status(chwsDrugData.status).json({ status: 201, data: 'No data found !' });
//   // }
// }
...
// function getChwsDrugQantity(ChwsDataFromDb: ChwsData[], Chw: Chws, fieldId: string, req: Request, res: Response): ChwsDrugQantityInfo {
...
//         const isDateValid: boolean = DateUtils.isBetween(`${start_date}`, data.reported_date, `${end_date}`);

//         if (isDateValid && idSourceValid && idDistrictValid && idSiteValid && idChwValid) {
          
//           const field = data.fields;

//           //field["h_drug_quantities"]["activity_date"];

          
//           if (data.form == "drug_quantities" && field.hasOwnProperty("h_drug_quantities") && field.hasOwnProperty("b_drug_quantities")) {
//             const operation =  field["h_drug_quantities"]["med_operation"];
//             const qty = field["b_drug_quantities"];
//             if (operation == "c_qty_received") out.month_quantity_received+=(notEmpty(qty[fieldId]) ? parseInt(qty[fieldId]) : 0);
//             if (operation == "c_qty_counted") out.inventory_quantity+=(notEmpty(qty[fieldId]) ? parseInt(qty[fieldId]) : 0);
//             if (operation == "c_qty_order") out.quantity_to_order+=(notEmpty(qty[fieldId]) ? parseInt(qty[fieldId]): 0);
//           }

//           if (data.form == "drug_movements") {
//             const movt =  field["h_drug_movements"];
//             const movementsType =  movt["med_type"];
//             const mvf = field["b_drug_movements"];

//             //movt["activity_date"];

//             if (movementsType == "c_med_loss") out.quantity_loss+=(notEmpty(mvf[fieldId]) ? parseInt(mvf[fieldId]) : 0);
//             if (movementsType == "c_med_expired") out.quantity_expired+=(notEmpty(mvf[fieldId]) ? parseInt(mvf[fieldId]) : 0);

//             if (movementsType == "c_med_borrowing") {
//               out.loan_borrowing_quantity+=(notEmpty(mvf[fieldId]) ? parseInt(mvf[fieldId]) : 0);
//               out.loan_borrowing_chws_code+=movt["med_chw_destination"];
//               out.loan_borrowing = 'Emprunt';
//             }
//             if (movementsType == "c_med_loan") {
//               out.loan_borrowing_quantity+=(notEmpty(mvf[fieldId]) ? parseInt(mvf[fieldId]) : 0);
//               out.loan_borrowing_chws_code+=movt["med_chw_destination"];
//               out.loan_borrowing = 'Prêt';
//             }

//             if (movementsType == "c_med_damaged") {
//               out.quantity_damaged+=(notEmpty(mvf[fieldId]) ? parseInt(mvf[fieldId]) : 0);
//             }
//             if (movementsType == "c_med_broken") {
//               out.quantity_broken+=(notEmpty(mvf[fieldId]) ? parseInt(mvf[fieldId]) : 0);
//             }
//             if (movementsType == "c_others") {
//               //movt["med_type_other"]
//               out.other_quantity+=(notEmpty(mvf[fieldId]) ? parseInt(mvf[fieldId]) : 0);
//             }
//             out.comments+=movt["med_comment"];
//           }
...
//           if (["lumartem", "amox_250", "amox_500", "para_250", "para_500", "zinc", "tdr", "sro"].includes(fieldId) && data.form == "pcime_c_asc") {
//             if (fieldId == "tdr" && field.hasOwnProperty("s_fever_child_TDR")) {
//               if (notEmpty(field["s_fever_child_TDR"]["s_fever_child_TDR_result"])) out.month_consumption+=1;
//             }

//             if (field.hasOwnProperty("s_drug_administered")) {
//               const dAdm = field["s_drug_administered"];
//               if(fieldId == "amox_250" && notEmpty(dAdm["c_amoxicilline_250_given"])) out.month_consumption+=1;
//               if(fieldId == "amox_500" && notEmpty(dAdm["c_amoxicilline_500_given"])) out.month_consumption+=1;
//               if(fieldId == "para_250" && notEmpty(dAdm["c_paracetamol_250_given"])) out.month_consumption+=1;
//               if(fieldId == "para_500" && notEmpty(dAdm["c_paracetamol_500_given"])) out.month_consumption+=1;
//               if(fieldId == "zinc" && notEmpty(dAdm["s_zinc_given"])) out.month_consumption+=1;
//               if(fieldId == "sro" && notEmpty(dAdm["s_ors_given"])) out.month_consumption+=1;
//               if(fieldId == "lumartem" && notEmpty(dAdm["s_lumartem_given"])) out.month_consumption+=1;
//               //dAdm["s_other_drug_given"]
//             }
//           }

//           if (data.form == "pregnancy_family_planning") {
//             if (fieldId == "pregnancy_test" && field.hasOwnProperty("s_reg_pregnancy_screen")) {
//               if (field["s_reg_pregnancy_screen.s_reg_urine_test"] == "yes") out.month_consumption+=1;
//             }

//             if (["pills","sayana"].includes(fieldId) && field.hasOwnProperty("s_fam_plan_screen")) {
//               if (field["s_fam_plan_screen.agreed_to_fp"] == "yes") {
//                 if (fieldId == "pills" && field["chosen_fp_method"] == 'oral_combination_pill') out.month_consumption+=1;
//                 if (fieldId == "sayana" && field["chosen_fp_method"] == 'injectible') out.month_consumption+=1;
//               }
//             }
//           }
          
//           if (["pills","sayana"].includes(fieldId) && data.form == "fp_follow_up_renewal") {
//             if (fieldId == "pills" && field["fp_method"] == 'oral_combination_pill') out.month_consumption+=1;
//             if (fieldId == "sayana" && field["fp_method"] == 'injectible') out.month_consumption+=1;
//           }
//         }
//       }
//     }
